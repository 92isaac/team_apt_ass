...
const ProductImageGallery = ({ images }) => {
  ...
  return (
    ...
    {/* <div className="pt-10 flex flex-row-reverse md:col-span-1 md:block"> */}
      ...
      <div className="w-1/4 md:w-full block md:flex md:justify-between mb-4 lg:mb-0">
        {images.map((image, index) => (
          <div
            key={index}
            className={`w-1/4 lg:w-full mb-4 object-cover lg:mb-0  ${
              selectedImage === image ? "border-2 border-blue-500" : ""
            }`}
            onClick={() => selectImage(image)}
          >
            <img src={image} alt="" className="w-1/4 h-auto" />
          </div>
        ))}
      </div>
